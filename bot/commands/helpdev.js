...
module.exports.run = async (client, message, args) => {
	...
	let cmds = cmdInfo.commands;

	let searchTerm = args[0];
	if(!searchTerm) return message.channel.send("Please provide a search term.");

	let matches = cmds.filter(c => c.name.toLowerCase().includes(searchTerm.toLowerCase()));

	let msgMatches = matches.map(c => c.name).join(", ");

	message.channel.send(msgMatches);
	console.log(msgMatches);
